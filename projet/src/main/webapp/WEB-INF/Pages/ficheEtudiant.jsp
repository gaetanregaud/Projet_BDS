	...
	<section id="informationetudiant">
		<c:if test="${user_id != null}">
			<div id="photo_id">
				...
				<c:if test="${!etudiant.photo.isEmpty()}">
					<img src="IMAGE/photo/etudiant/${etudiant.photo}.jpg" width="250px"
						height="250px">
				</c:if>
			</div>
			<div id="titre-h3">
			...
			<div id="information">
				<fieldset>
					...
					<p>
						Cotisation :
						<c:if test="${etudiant.isCotisation() == true}">
						...
						, Certificat:
						<c:if test="${etudiant.isCertificat() == true}">
							<span class="oui">Oui</span>
						</c:if>
			...
			<div id="pratiquer_note">
				<fieldset>
					<legend>
						Sport Noté (<span class="btn"><a id="montrer_pratnote">montrer</a>/<a
							id="cacher_pratnote">cacher</a></span>)
					</legend>
					<div id="table_sportnote">
								...
								<c:forEach var="pratiquer" items="${pratiquers}">
									<c:if test="${pratiquer.note == 1}">
										<tr>
											...
											<td><c:if test="${pratiquer.note == 1}">
													<span class="oui">Oui</span>
												</c:if>
												<c:if test="${pratiquer.note == 0}">
													<span class="non">Non</span>
												</c:if>
												<c:if test="${pratiquer.note == -1}">
													<span class="">NI</span>
						...
						<br /> <input type="button" id="afficher_seance"
							value="Afficher séance" /> <a href="notervp" target="_bank"><input
							type="button" id="notervp" value="Noter VP" /></a>
					</div>
					<div id="table_seance">
						<table>
							<thead>
							...
							<tbody>
								<c:forEach var="seance" items="${seances}">
									<tr>
										...
										<td><fmt:formatDate value="${seance.date_seance}"
												pattern="dd MMMM yyy" /></td>
										<td><c:if test="${seance.presence_seance == 1}">
												<span class="oui">Oui</span>
											</c:if>
											<c:if test="${seance.presence_seance == 0}">
												<span class="non">Non</span>
											</c:if></td>
									</tr>
								</c:forEach>
						...
						<br /> <input type="button" id="cacher_seance"
							value="Cacher séance" />
					</div>
				</fieldset>
			</div>
			...
			<div id="challengeDo">
				<fieldset>
					<legend>
						Challenge(s) (<span class="btn"><a id="montrer_challenge">montrer</a>/<a
							id="cacher_challenge">cacher</a></span>)
					</legend>
